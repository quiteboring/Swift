...
class MovementDescend(from: BlockPos, to: BlockPos) : Movement(from, to) {
  ...
  companion object {
    fun calculateCost(
      ...
      if (!MovementHelper.isPassable(ctx.bsa, destX, y, destZ)) return
      if (!MovementHelper.isPassable(ctx.bsa, destX, y + 1, destZ)) return

      for (fallDist in 1..ctx.maxFallHeight) {
        val destY = y - fallDist

        if (!MovementHelper.isPassable(ctx.bsa, destX, destY + 1, destZ)) return
        if (!MovementHelper.isPassable(ctx.bsa, destX, destY, destZ)) return

        if (MovementHelper.isSolid(ctx.bsa, destX, destY - 1, destZ)) {
